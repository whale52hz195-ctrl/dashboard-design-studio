...
export default function Settings() {
  ...
  const handleDeleteReason = (id: string) => {
    if (!settings) return;
    setSettings({ ...settings, reportReasons: settings.reportReasons.filter(r => r.id !== id) });
  };

  const handleDeleteCurrency = (id: string) => {
    if (!settings) return;
    setSettings({ ...settings, currencies: settings.currencies.filter(c => c.id !== id) });
  };
            ...
            <SettingSection title="Stripe Setting">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SettingField label="Publishable Key">
                  <Input className={inputClass} value={settings.payment.stripePublishableKey} onChange={e => setSettings({ ...settings, payment: { ...settings.payment, stripePublishableKey: e.target.value } })} placeholder="pk_..." />
                </SettingField>
                <SettingField label="Secret Key">
                  <Input className={inputClass} type="password" value={settings.payment.stripeSecretKey} onChange={e => setSettings({ ...settings, payment: { ...settings.payment, stripeSecretKey: e.target.value } })} placeholder="sk_..." />
                </SettingField>
              </div>
            </SettingSection>
            ...
            <SettingSection title="Razorpay Setting">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SettingField label="Razorpay ID">
                  <Input className={inputClass} value={settings.payment.razorpayId} onChange={e => setSettings({ ...settings, payment: { ...settings.payment, razorpayId: e.target.value } })} placeholder="rzp_..." />
                </SettingField>
                <SettingField label="Secret Key">
                  <Input className={inputClass} type="password" value={settings.payment.razorpaySecretKey} onChange={e => setSettings({ ...settings, payment: { ...settings.payment, razorpaySecretKey: e.target.value } })} placeholder="Enter Secret Key" />
                </SettingField>
              </div>
            </SettingSection>
            ...
            <SettingSection title="Google Play Setting">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SettingField label="Service Account Email">
                  <Input className={inputClass} value={settings.payment.googlePlayServiceEmail} onChange={e => setSettings({ ...settings, payment: { ...settings.payment, googlePlayServiceEmail: e.target.value } })} placeholder="service@project.iam.gserviceaccount.com" />
                </SettingField>
                <SettingField label="Private Key">
                  <Input className={inputClass} type="password" value={settings.payment.googlePlayPrivateKey} onChange={e => setSettings({ ...settings, payment: { ...settings.payment, googlePlayPrivateKey: e.target.value } })} placeholder="Enter Private Key" />
                </SettingField>
              </div>
            </SettingSection>
          </TabsContent>
            ...
            <SettingSection title="Sightengine Configuration">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SettingField label="User">
                  <Input className={inputClass} value={settings.contentModeration.sightengineUser} onChange={e => setSettings({ ...settings, contentModeration: { ...settings.contentModeration, sightengineUser: e.target.value } })} placeholder="Enter Sightengine User" />
                </SettingField>
                <SettingField label="API Secret">
                  <Input className={inputClass} type="password" value={settings.contentModeration.sightengineApiSecret} onChange={e => setSettings({ ...settings, contentModeration: { ...settings.contentModeration, sightengineApiSecret: e.target.value } })} placeholder="Enter API Secret" />
                </SettingField>
              </div>
            </SettingSection>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SettingField label="Video Banned Keywords">
                  <Select>
                    ...
                    <SelectContent>
                      {settings.contentModeration.videoBannedKeywords.map((keyword, index) => (
                        <SelectItem key={index} value={keyword}>{keyword}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </SettingField>
                <SettingField label="Post Banned Keywords">
                  <Select>
                    ...
                    <SelectContent>
                      {settings.contentModeration.postBannedKeywords.map((keyword, index) => (
                        <SelectItem key={index} value={keyword}>{keyword}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
            ...
            <SettingSection title="Minimum Coin Setting">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <SettingField label="Currency">
                  <Select value={settings.withdrawal.currency} onValueChange={v => setSettings({ ...settings, withdrawal: { ...settings.withdrawal, currency: v } })}>
                    <SelectTrigger className={inputClass}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <SettingField label="Coins">
                  <Input className={inputClass} value={settings.withdrawal.coins} onChange={e => setSettings({ ...settings, withdrawal: { ...settings.withdrawal, coins: e.target.value } })} />
                </SettingField>
                <SettingField label="Min Coins Payout (User)">
                  <Input className={inputClass} value={settings.withdrawal.minCoinsPayoutUser} onChange={e => setSettings({ ...settings, withdrawal: { ...settings.withdrawal, minCoinsPayoutUser: e.target.value } })} />
                </SettingField>
                <SettingField label="Min Coins Payout (Agency)">
                  <Input className={inputClass} value={settings.withdrawal.minCoinsPayoutAgency} onChange={e => setSettings({ ...settings, withdrawal: { ...settings.withdrawal, minCoinsPayoutAgency: e.target.value } })} />
                </SettingField>
              </div>
            </SettingSection>
          </TabsContent>
                ...
                {[1, 2, 3, 4, 5].map(i => (
                  <div key={i} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <SettingField label={`Bet ${i} (Coin)`}>
                      <Input className={inputClass} value={settings.game.bets[i.toString()] || ''} onChange={e => setSettings({ ...settings, game: { ...settings.game, bets: { ...settings.game.bets, [i.toString()]: e.target.value } } })} />
                    </SettingField>
                    <SettingField label={`Bet ${i} Description`}>
                      <Input className={inputClass} placeholder={`Description for bet ${i}`} />
                    </SettingField>
                  </div>
                ))}
              </div>
